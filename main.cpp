// Grocery shop Billing system project created by @mohit singh tanwar
...
m:
    ...
    case 1:
        ...
        if (email == "mohit@admin.com" && password == "mohit@#123")
        {
            administrator();
        }
        else
...
m:
    ...
    if (!data) // if this doesn't exist
    {
        ...
        // to write in file "database.txt", with the help of object data of Mohit Singh Tanwar
    ...
    else
    {
        ...
        while (!data.eof())
        {
            if (pkey == pcode) // if entered code matches with code inside file database.txt
            {
                ...
                cout << "\n\n\t\t Record edited ";
                token++; // to mark that record with this code exist in the file of mohit
            }
